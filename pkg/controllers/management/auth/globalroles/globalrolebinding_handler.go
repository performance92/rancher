...
func (grb *globalRoleBindingLifecycle) reconcileGlobalRoleBinding(globalRoleBinding *v3.GlobalRoleBinding, localConditions *[]metav1.Condition) error {
	...
	if globalRoleBinding.GlobalRoleName == rbac.GlobalRestrictedAdmin {
		if err := grb.syncDownstreamClusterPermissions(subject, globalRoleBinding); err != nil {
			grb.status.AddCondition(localConditions, condition, failedToSyncDownstreamClusterPermissions, err)
			return err
		}
	}
	...
	if crb != nil {
		...
		return grb.addRulesForTemplateAndTemplateVersions(globalRoleBinding, subject, condition, localConditions)
	}
...
func (grb *globalRoleBindingLifecycle) addRulesForTemplateAndTemplateVersions(globalRoleBinding *v3.GlobalRoleBinding, subject v1.Subject, condition metav1.Condition, localConditions *[]metav1.Condition) error {
	...
	if err != nil && !apierrors.IsNotFound(err) {
		grb.status.AddCondition(localConditions, condition, failedToGetGlobalRoleBinding, err)
		return err
	}
	isCatalogRole := false
	if gr != nil {
	...
	if isCatalogRole {
		...
		if err != nil {
			grb.status.AddCondition(localConditions, condition, failedToGetRole, err)
			return err
		}
		...
		if err != nil {
			if apierrors.IsNotFound(err) {
				...
				if err != nil && !apierrors.IsAlreadyExists(err) {
					grb.status.AddCondition(localConditions, condition, failedToCreateRoleBinding, err)
					return err
				}
			} else {
				grb.status.AddCondition(localConditions, condition, failedToGetRoleBinding, err)
				return err
			}
		}
	}
