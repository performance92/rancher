...
var (
	...
	roleName            = namegen.AppendRandomString("bro-role")
...
func createAndValidateBackup(client *rancher.Client, bucket string) (*v1.SteveAPIObject, string, error) {
...
func verifyRancherResources(client *rancher.Client, userList []*management.User, projList []*management.Project, roleList []*management.RoleTemplate) (error, error, error) {
	...
	for i := range projList {
		_, err := client.Steve.SteveType("management.cattle.io.project").ByID(projList[i].ID)
		if err != nil {
			return nil, err, nil
		}
	}
	...
	for i := range roleList {
		_, err := client.Steve.SteveType("management.cattle.io.roletemplate").ByID(roleList[i].ID)
		if err != nil {
			return nil, nil, err
		}
	}
