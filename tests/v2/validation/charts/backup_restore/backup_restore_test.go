...
func (b *BackupTestSuite) TestS3InPlaceRestore() {
	...
	rke1ClusterObj, _, rke1ClusterConfig, err := createDownstreamCluster(b.client, "RKE1")
	require.NoError(b.T(), err)

	logrus.Info("Provisioning a downstream RKE2 cluster...")
	_, rke2SteveObj, rke2ClusterConfig, err := createDownstreamCluster(b.client, "RKE2")
	require.NoError(b.T(), err)
	...
	_, backupFileName, err := createBackup(b.client, s3StorageLocation)
	require.NoError(b.T(), err)
